...
async fn ac_coupled_battery_charge_limit_capped_at_3kw() {
    ...
    assert_eq!(
        val, 30,
        "HR 208 (battery_max_charge_kw) should be 30 (= 3.0 kW / 0.1 scaling)"
    );
}
...
async fn charge_slot_1_reads_default_schedule() {
    ...
    assert_eq!(
        val94, 60,
        "HR 94 (charge_slot_1_start) should be 60 (disabled sentinel)"
    );
    ...
    assert_eq!(
        val95, 60,
        "HR 95 (charge_slot_1_end) should be 60 (disabled sentinel)"
    );
}
...
async fn discharge_slot_1_disabled_by_default() {
    ...
    assert_eq!(
        val56, 60,
        "HR 56 (discharge_slot_1_start) should be 60 (disabled)"
    );

    let val57 = read_hr(&mut stream, 57).await;
    assert_eq!(
        val57, 60,
        "HR 57 (discharge_slot_1_end) should be 60 (disabled)"
    );
}
...
async fn charge_slot_2_disabled_by_default() {
    ...
    assert_eq!(
        val31, 60,
        "HR 31 (charge_slot_2_start) should be 60 (disabled)"
    );

    let val32 = read_hr(&mut stream, 32).await;
    assert_eq!(
        val32, 60,
        "HR 32 (charge_slot_2_end) should be 60 (disabled)"
    );
}
...
async fn write_charge_slot_1() {
    ...
    assert_eq!(
        ...
        "Write should succeed for HR 94 (ReadWrite)"
    );
...
async fn write_discharge_slot_1() {
    ...
    assert_eq!(
        func, FC_WRITE_SINGLE,
        "Write should succeed for HR 56 (ReadWrite)"
    );
...
async fn full_input_register_block_0_to_59() {
    ...
    assert_eq!(
        data[5], 2400,
        "IR 5 = grid voltage should be 2400 (= 240.0V × 10)"
    );

    // IR 18 (PV1 power) > 0 (solar generating)
    assert!(
        data[18] > 0,
        "IR 18 = PV1 power should be > 0 (got {})",
        data[18]
    );
    ...
    assert!(
        data[59] > 0,
        "IR 59 = battery SOC should be > 0 (got {})",
        data[59]
    );
}
...
async fn full_holding_register_block_0_to_119() {
    ...
    assert_eq!(
        data[0], 0x2001,
        "HR 0 (device_type) should be 0x2001 for Gen3Hybrid"
    );
...
async fn multi_battery_bms_slave_32_and_33() {
    ...
    assert!(
        cell_sum_0 > 0,
        "Battery 0 cell voltages should be non-zero (sum = {})",
        cell_sum_0
    );
    ...
    assert!(
        cell_sum_1 > 0,
        "Battery 1 cell voltages should be non-zero (sum = {})",
        cell_sum_1
    );
    ...
    assert_ne!(
        temp_0, temp_1,
        "Battery temperatures should differ: {} vs {}",
        temp_0, temp_1
    );
}
